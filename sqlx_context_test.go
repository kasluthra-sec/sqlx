...
func loadDefaultFixtureContext(ctx context.Context, db *DB, t *testing.T) {
	...
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO person (first_name, last_name, email) VALUES (?, ?, ?)")), "Jason", "Moiron", "jmoiron@jmoiron.net")
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO person (first_name, last_name, email) VALUES (?, ?, ?)")), "John", "Doe", "johndoeDNE@gmail.net")
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO place (country, city, telcode) VALUES (?, ?, ?)")), "United States", "New York", "1")
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO place (country, telcode) VALUES (?, ?)")), "Hong Kong", "852")
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO place (country, telcode) VALUES (?, ?)")), "Singapore", "65")
	if db.DriverName() == "mysql" {
		tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO capplace (`COUNTRY`, `TELCODE`) VALUES (?, ?)")), "Sarf Efrica", "27")
	} else {
		tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO capplace (\"COUNTRY\", \"TELCODE\") VALUES (?, ?)")), "Sarf Efrica", "27")
	}
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO employees (name, id) VALUES (?, ?)")), "Peter", "4444")
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO employees (name, id, boss_id) VALUES (?, ?, ?)")), "Joe", "1", "4444")
	tx.MustExecContext(ctx, tx.Rebind(safesql.New("INSERT INTO employees (name, id, boss_id) VALUES (?, ?, ?)")), "Martin", "2", "4444")
	tx.Commit()
}
...
func TestMissingNamesContextContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		err := db.SelectContext(ctx, &pps, safesql.New("SELECT * FROM person"))
		if err == nil {
		...
		// test Get
		pp := PersonPlus{}
		err = db.GetContext(ctx, &pp, safesql.New("SELECT * FROM person LIMIT 1"))
		if err == nil {
		...
		pps = []PersonPlus{}
		rows, err := db.QueryContext(ctx, safesql.New("SELECT * FROM person LIMIT 1"))
		if err != nil {
			t.Fatal(err)
		}
		rows.Next()
		...
		// test naked StructScan
		pps = []PersonPlus{}
		rowsx, err := db.QueryxContext(ctx, safesql.New("SELECT * FROM person LIMIT 1"))
		if err != nil {
			t.Fatal(err)
		}
		rowsx.Next()
		...
		nstmt, err := db.PrepareNamedContext(ctx, safesql.New(`SELECT * FROM person WHERE first_name != :name`))
		if err != nil {
			t.Fatal(err)
		}
...
func TestEmbeddedStructsContextContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		// test "deep nesting"
		l3s := []Loop3{}
		err = db.SelectContext(ctx, &l3s, safesql.New(`select * from person`))
		if err != nil {
			t.Fatal(err)
		}
		for _, l3 := range l3s {
		...
		ec := []EmbedConflict{}
		err = db.SelectContext(ctx, &ec, safesql.New(`select * from person`))
...
func TestSelectSliceMapTimeContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		loadDefaultFixtureContext(ctx, db, t)
		rows, err := db.QueryxContext(ctx, safesql.New("SELECT * FROM person"))
		if err != nil {
			t.Fatal(err)
		}
		for rows.Next() {
...
func TestNilReceiverContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		loadDefaultFixtureContext(ctx, db, t)
		var p *Person
		err := db.GetContext(ctx, p, safesql.New("SELECT * FROM person LIMIT 1"))
		if err == nil {
			t.Error("Expected error when getting into nil struct ptr.")
		}
		var pp *[]Person
		err = db.SelectContext(ctx, pp, safesql.New("SELECT * FROM person"))
		if err == nil {
...
func TestNamedQueryContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		p2 := &Person{}
		rows, err := db.NamedQueryContext(ctx, safesql.New("SELECT * FROM person WHERE first_name=:first_name"), p)
		if err != nil {
			log.Fatal(err)
		}
		for rows.Next() {
		...
		rows, err = db.NamedQueryContext(ctx, pdb(safesql.New(`
			SELECT * FROM jsperson
			WHERE
				"FIRST"=:FIRST AND
				last_name=:last_name AND
		...
		pp2 := &PlacePerson{}
		rows, err = db.NamedQueryContext(ctx, safesql.New(`
			SELECT
				first_name,
				last_name,
				email,
...
func TestNilInsertsContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		db.MustExecContext(ctx, r(safesql.New(`INSERT INTO tt (id) VALUES (1)`)))
		db.GetContext(ctx, &v, r(safesql.New(`SELECT * FROM tt`)))
		if v.ID != 1 {
		...
		db.NamedExecContext(ctx, safesql.New(`INSERT INTO tt (id, value) VALUES (:id, :value)`), v)

		db.GetContext(ctx, &v2, r(safesql.New(`SELECT * FROM tt WHERE id=2`)))
		if v.ID != v2.ID {
...
func TestScanErrorContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		rows, err := db.QueryxContext(ctx, safesql.New("SELECT * FROM kv"))
		if err != nil {
			t.Error(err)
		}
		for rows.Next() {
...
func TestUsageContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		err := db.SelectContext(ctx, &slicemembers, safesql.New("SELECT * FROM place ORDER BY telcode ASC"))
		if err != nil {
			t.Fatal(err)
		}
		...
		err = db.SelectContext(ctx, &people, safesql.New("SELECT * FROM person ORDER BY first_name ASC"))
		if err != nil {
			t.Fatal(err)
		}
		...
		err = db.GetContext(ctx, &jason, db.Rebind(safesql.New("SELECT * FROM person WHERE first_name=?")), "Foobar")
		if err == nil {
		...
		stmt1, err := db.PreparexContext(ctx, db.Rebind(safesql.New("SELECT * FROM person WHERE first_name=?")))
		if err != nil {
			t.Fatal(err)
		}
		jason = Person{}
		...
		stmt2, err := db.PreparexContext(ctx, db.Rebind(safesql.New("SELECT * FROM person WHERE first_name=?")))
		if err != nil {
			t.Fatal(err)
		}
		jason = Person{}
		...
		places := []*Place{}
		err = db.SelectContext(ctx, &places, safesql.New("SELECT telcode FROM place ORDER BY telcode ASC"))
		if err != nil {
			t.Fatal(err)
		}
		...
		placesptr := []PlacePtr{}
		err = db.SelectContext(ctx, &placesptr, safesql.New("SELECT * FROM place ORDER BY telcode ASC"))
		if err != nil {
			t.Error(err)
		}
		...
		places2 := []Place{}
		err = db.SelectContext(ctx, &places2, safesql.New("SELECT * FROM place ORDER BY telcode ASC"))
		if err != nil {
			t.Fatal(err)
		}
		...
		p := Place{}
		err = db.SelectContext(ctx, &p, safesql.New("SELECT * FROM place ORDER BY telcode ASC"))
		if err == nil {
		...
		pl := []Place{}
		err = db.SelectContext(ctx, pl, safesql.New("SELECT * FROM place ORDER BY telcode ASC"))
		if err == nil {
		...
		stmt, err := db.PreparexContext(ctx, db.Rebind(safesql.New("SELECT country, telcode FROM place WHERE telcode > ? ORDER BY telcode ASC")))
		if err != nil {
			t.Error(err)
		}
		...
		rows, err := db.QueryxContext(ctx, safesql.New("SELECT * FROM place"))
		if err != nil {
			t.Fatal(err)
		}
		place := Place{}
		...
		_, err = db.NamedExecContext(ctx, safesql.New("INSERT INTO person (first_name, last_name, email) VALUES (:first, :last, :email)"), map[string]interface{}{
			"first": "Bin",
			"last":  "Smuth",
		...
		rows, err = db.NamedQueryContext(ctx, safesql.New("SELECT * FROM person WHERE first_name=:first"), map[string]interface{}{"first": "Bin"})
		if err != nil {
			t.Fatal(err)
		}
		...
		_, err = db.NamedExecContext(ctx, safesql.New("INSERT INTO person (first_name, last_name, email) VALUES (:first_name, :last_name, :email)"), ben)

		if err != nil {
			t.Fatal(err)
		}

		rows, err = db.NamedQueryContext(ctx, safesql.New("SELECT * FROM person WHERE first_name=:first_name"), ben)
		if err != nil {
			t.Fatal(err)
		}
		for rows.Next() {
		...
		person := &Person{}
		err = db.GetContext(ctx, person, safesql.New("SELECT * FROM person WHERE first_name=$1"), "does-not-exist")
		if err == nil {
		...
		stmt, err = db.PreparexContext(ctx, db.Rebind(safesql.New("SELECT * FROM person WHERE first_name=?")))
		if err != nil {
			t.Fatal(err)
		}
		...
		rsa := CPlace{}
		err = db.GetContext(ctx, &rsa, safesql.New("SELECT * FROM capplace;"))
		if err != nil {
		...
		err = dbCopy.GetContext(ctx, &rsa, safesql.New("SELECT * FROM capplace;"))
		if err != nil {
			fmt.Println(db.DriverName())
			t.Error(err)
		}

		err = db.GetContext(ctx, &rsa, safesql.New("SELECT * FROM cappplace;"))
		if err == nil {
		...
		var sdest []string
		rows, err = db.QueryxContext(ctx, safesql.New("SELECT email FROM person ORDER BY email ASC;"))
		if err != nil {
			t.Error(err)
		}
		...
		var count int
		err = db.GetContext(ctx, &count, safesql.New("SELECT count(*) FROM person;"))
		if err != nil {
			t.Error(err)
		}
		if count != len(sdest) {
		...
		err = db.GetContext(ctx, &addedAt, safesql.New("SELECT added_at FROM person LIMIT 1;"))
		if err != nil {
			t.Error(err)
		}

		var addedAts []time.Time
		err = db.SelectContext(ctx, &addedAts, safesql.New("SELECT added_at FROM person;"))
		if err != nil {
			t.Error(err)
		}
		...
		var pcount *int
		err = db.GetContext(ctx, &pcount, safesql.New("SELECT count(*) FROM person;"))
		if err != nil {
			t.Error(err)
		}
		if *pcount != count {
		...
		// test Select...
		sdest = []string{}
		err = db.SelectContext(ctx, &sdest, safesql.New("SELECT first_name FROM person ORDER BY first_name ASC;"))
		if err != nil {
			t.Error(err)
		}
		...
		err = db.SelectContext(ctx, &nsdest, safesql.New("SELECT city FROM place ORDER BY city ASC"))
		if err != nil {
			t.Error(err)
		}
...
func TestEmbeddedMapsContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		var count int
		err := db.GetContext(ctx, &count, safesql.New("SELECT count(*) FROM message"))
		if err != nil {
			t.Fatal(err)
		}
		...
		var m Message
		err = db.GetContext(ctx, &m, safesql.New("SELECT * FROM message LIMIT 1;"))
		if err != nil {
			t.Fatal(err)
		}
		if m.Properties == nil {
...
func TestIssue197Context(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		var err error
		var v, q Var
		if err = db.GetContext(ctx, &v, safesql.New(`SELECT '{"a": "b"}' AS raw`)); err != nil {
			t.Fatal(err)
		}
		if err = db.GetContext(ctx, &q, safesql.New(`SELECT 'null' AS raw`)); err != nil {
			t.Fatal(err)
		}

		var v2, q2 Var2
		if err = db.GetContext(ctx, &v2, safesql.New(`SELECT '{"a": "b"}' AS raw`)); err != nil {
			t.Fatal(err)
		}
		if err = db.GetContext(ctx, &q2, safesql.New(`SELECT 'null' AS raw`)); err != nil {
			t.Fatal(err)
		}

		var v3, q3 Var3
		if err = db.QueryRowContext(ctx, safesql.New(`SELECT '{"a": "b"}' AS raw`)).Scan(&v3.Raw); err != nil {
			t.Fatal(err)
		}
		if err = db.QueryRowContext(ctx, safesql.New(`SELECT '{"c": "d"}' AS raw`)).Scan(&q3.Raw); err != nil {
			t.Fatal(err)
		}
		t.Fail()
	})
...
func TestEmbeddedLiteralsContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		db.MustExecContext(ctx, db.Rebind(safesql.New("INSERT INTO x (k) VALUES (?), (?), (?);")), "one", "two", "three")

		target := t1{}
		err := db.GetContext(ctx, &target, db.Rebind(safesql.New("SELECT * FROM x WHERE k=?")), "one")
		if err != nil {
			t.Error(err)
		}
		if *target.K != "one" {
		...
		target2 := t2{}
		err = db.GetContext(ctx, &target2, db.Rebind(safesql.New("SELECT * FROM x WHERE k=?")), "one")
		if err != nil {
			t.Error(err)
		}
		if *target2.K != "one" {
...
func TestConn(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		_, err = conn.ExecContext(ctx, conn.Rebind(safesql.New(`INSERT INTO tt_conn (id, value) VALUES (?, ?), (?, ?)`)), 1, "a", 2, "b")
		if err != nil {
			t.Fatal(err)
		}
		...
		err = conn.SelectContext(ctx, &v, safesql.New("SELECT * FROM tt_conn ORDER BY id ASC"))
		if err != nil {
			t.Fatal(err)
		}
		...
		v1 := s{}
		err = conn.GetContext(ctx, &v1, conn.Rebind(safesql.New("SELECT * FROM tt_conn WHERE id=?")), 1)
		...
		stmt, err := conn.PreparexContext(ctx, conn.Rebind(safesql.New("SELECT * FROM tt_conn WHERE id=?")))
		if err != nil {
			t.Fatal(err)
		}
		v1 = s{}
		...
		rows, err := conn.QueryxContext(ctx, safesql.New("SELECT * FROM tt_conn"))
		if err != nil {
			t.Fatal(err)
		}
